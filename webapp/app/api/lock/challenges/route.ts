...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!profile.Extensions.ChallengeProgression) profile.Extensions.ChallengeProgression = {}

    const prog = profile.Extensions.ChallengeProgression

    if (ids && Array.isArray(ids) && ids.length > 0) {
      // Lock specific challenges
      let lockedCount = 0
      for (const id of ids) {
        if (prog[id]) {
          delete prog[id]
      ...
      await logActivity(peacockPath, `Locked ${lockedCount} challenge${lockedCount !== 1 ? 's' : ''}`, "unlock")

      return NextResponse.json({
        success: true,
        message: `Successfully locked ${lockedCount} challenge${lockedCount !== 1 ? 's' : ''}`
      })
    } else {
      // Lock all challenges
      ...
      await writeJsonFile(profileFile, profile)
      await logActivity(peacockPath, `Locked all ${totalCount} challenges`, "unlock")

      return NextResponse.json({
        success: true,
        message: `Successfully locked all ${totalCount} challenges`
      })
    }
  } catch (error) {
    return NextResponse.json(
