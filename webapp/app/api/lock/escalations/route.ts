...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!profile.Extensions.PeacockEscalations) profile.Extensions.PeacockEscalations = {}
    if (!profile.Extensions.PeacockCompletedEscalations) profile.Extensions.PeacockCompletedEscalations = []

    if (ids && Array.isArray(ids) && ids.length > 0) {
      // Lock specific escalations
      let lockedCount = 0
      for (const id of ids) {
        if (profile.Extensions.PeacockEscalations[id]) {
          delete profile.Extensions.PeacockEscalations[id]
          lockedCount++
        }
        const index = profile.Extensions.PeacockCompletedEscalations.indexOf(id)
        if (index > -1) {
          profile.Extensions.PeacockCompletedEscalations.splice(index, 1)
        }
      }

      await writeJsonFile(profileFile, profile)
      await logActivity(peacockPath, `Locked ${lockedCount} escalation${lockedCount !== 1 ? 's' : ''}`, "unlock")

      return NextResponse.json({
        success: true,
        message: `Successfully locked ${lockedCount} escalation${lockedCount !== 1 ? 's' : ''}`
      })
    } else {
      // Lock all escalations
      const totalCount = Object.keys(profile.Extensions.PeacockEscalations).length
      profile.Extensions.PeacockEscalations = {}
      profile.Extensions.PeacockCompletedEscalations = []

      await writeJsonFile(profileFile, profile)
      await logActivity(peacockPath, `Locked all ${totalCount} escalations`, "unlock")

      return NextResponse.json({
        success: true,
        message: `Successfully locked all ${totalCount} escalations`
      })
    }
  } catch (error) {
    return NextResponse.json(
