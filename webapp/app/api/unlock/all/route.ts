...
async function getAllEscalationIds(peacockPath: string) {
  ...
  if (escalationsData) {
    for (const escList of Object.values(escalationsData)) {
  ...
  return { progress, completed: Array.from(completedSet) }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Max profile level
    prog.ProfileLevel = GAME_CONSTANTS.MAX_LEVEL
    prog.XP = GAME_CONSTANTS.MAX_XP
    if (!prog.PlayerProfileXP) prog.PlayerProfileXP = {}
    prog.PlayerProfileXP.Total = GAME_CONSTANTS.MAX_XP
    prog.PlayerProfileXP.ProfileLevel = GAME_CONSTANTS.MAX_LEVEL

    // Max Merces
    if (!prog.Merces) prog.Merces = {}
    prog.Merces.Total = GAME_CONSTANTS.MAX_MERCES
    prog.Merces.ProfileLevel = GAME_CONSTANTS.MAX_PRESTIGE
    ...
    profile.Extensions.PeacockEscalations = escProgress
    profile.Extensions.PeacockCompletedEscalations = escCompleted

    // Max Freelancer
    if (!profile.Extensions.CPD) profile.Extensions.CPD = {}
    if (!profile.Extensions.CPD[GAME_CONSTANTS.FREELANCER_ID]) profile.Extensions.CPD[GAME_CONSTANTS.FREELANCER_ID] = {}
    profile.Extensions.CPD[GAME_CONSTANTS.FREELANCER_ID].MyMoney = GAME_CONSTANTS.MAX_MERCES
    profile.Extensions.CPD[GAME_CONSTANTS.FREELANCER_ID].EvergreenLevel = GAME_CONSTANTS.MAX_PRESTIGE

    await writeJsonFile(profileFile, profile)

    await logActivity(peacockPath, "Unlocked all content, max mastery, and level", "profile")

    return NextResponse.json({ success: true, message: "Unlocked everything successfully" })
  } catch (error) {
