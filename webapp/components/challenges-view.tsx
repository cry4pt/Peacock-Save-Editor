...
import { unlockChallenges, lockChallenges } from "@/lib/api";
...
export function ChallengesView() {
  ...
  const handleLockSelected = async () => {
    ...
    setLocking(true);
    try {
      const result = await lockChallenges(selectedItems);
      toast.success(result.message);
      setSelectedItems([]);
      mutate();
      mutateActivities();
  ...
  const handleLockAll = async () => {
    setLocking(true);
    try {
      const result = await lockChallenges();
      toast.success(result.message);
      mutate();
      mutateActivities();
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to lock challenges",
      );
    } finally {
      setLocking(false);
    }
  ...
  const handleLockSingle = async (id: string) => {
    try {
      const result = await lockChallenges([id]);
      toast.success(result.message);
      mutate();
      mutateActivities();
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to lock challenge",
      );
    }
  };
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <h3 className={`font-medium text-foreground transition-colors ${
                        challenge.completed ? "group-hover:text-destructive" : "group-hover:text-primary"
                      }`}>
                        {challenge.name}
                      </h3>
